...
def training(choice='n'):
    if choice == 'y':        
        FOLD_NO=5
        INITIAL_NEURAL_ACTIVITY = [0.95] #np.arange(0.01, 0.99, 0.01)
        DISCRIMINATION_THRESHOLD = [0.499] #[0.499,0.969]
        EPSILON = [0.305] #np.arange(0.001,0.5000,0.001)
...
clf = DecisionTreeClassifier(min_samples_leaf=MSL, max_depth=MD, random_state=7, ccp_alpha=CCP)
