    ...
    if choice == 'y':
        ...
        for C in C_values:
            for solver in solvers:
                ...
                FSCORE_TEMP = []
                clf = LogisticRegression(C=C, solver=solver, n_jobs=2, random_state=42)
                ...
                print(f'Mean F1 score for C={C}, solver={solver} is = {mean_f1}')
        ...
        np.save(RESULT_PATH+"/h_SOLVER.npy", np.array([BESTSOLVER], dtype=object))
...
SOLVER = np.load(RESULT_PATH+"/h_SOLVER.npy", allow_pickle=True)[0]
F1SCORE = np.load(RESULT_PATH+"/h_F1SCORE.npy")[0]


clf = LogisticRegression(C=C, solver=SOLVER, random_state=42, max_iter=1000)
