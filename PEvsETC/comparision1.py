...
def correlation(a_values, transient, D, t, bins):
    test_case = np.arange(10000,100000,10000)
    cases = len(test_case)
    case = 1
    print("Progress :\n")
...
def plotting_with_noise(a_values, results_dict):
    ...
    for i, (noise, (lyap, perm, etc)) in enumerate(results_dict.items()):
        if i < len(results_dict):
            ...
            axes[0, 1].plot(a_values, perm, linewidth=1.5, label=label, alpha=0.8)
    ...
    for i, (noise, (lyap, perm, etc)) in enumerate(results_dict.items()):
        if i < len(results_dict):
            label = f'Noise σ={noise}' if noise > 0 else 'Clean'
            axes[1, 0].plot(a_values, etc, linewidth=1.5, label=label, alpha=0.8)
